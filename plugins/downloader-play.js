...
❏ 👀 *Vizualiações:* ${`${MilesNumber(yt_play[0].views)}`}
...
❏ *_Enviando ${additionalText}, aguarde um momento．．．_*`.trim();
                ...
                try {
                  ...
                  try {
                    ...
                    try {
                      ...
                      await conn.reply(m.chat, '*[❗] Error, não foi possivel baixar o audio.*', m);
                    }
                  }
                }
              }
    ...
    if (command == 'play2') {
      try {
        ...
        if (size >= limit2) {
          await conn.sendMessage(m.chat, {text: `*[ ✔ ] BAIXE SEU VIDEO EM ${dl_url}*`}, {quoted: m});
          return;
        }
